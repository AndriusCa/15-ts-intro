...
export class Shop {
  ...
  sellProduct(productId: number, amount: number): [boolean, string] {
    ...
    if (!isSold) {
      return [
        true,
        `❌ Nera norimo ${foundProduct.getName()} kiekio: nori ${amount}; turim ${foundProduct.getAmount()}.`,
      ]
    }
  ...
  summary() {
    ...
    if (this.inventor.length) {
      ...
      for (const { name, acquisitionPrice, sellingPrice, amount } of this
        .inventor) {
        listStrings.push(
          `${++i}) ${name}: ${acquisitionPrice}; ${sellingPrice}; ${amount};`
        )
      }
